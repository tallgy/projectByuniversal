						...
						<view class="item-content">
							<view v-if="type=='fill'">
								<picker mode="date" @change="datePickerChange($event, detailItem.textName)">
									...
									<text class="item-ct-unit arrowBottom"></text>	<!-- v-if="(itemValue[detailItem.textName]=='' || itemValue[detailItem.textName]==null)"-->
									<!-- <text class="item-ct-unit fork" @click.stop="itemValue[detailItem.textName]=null" v-else></text> -->
								</picker>
							</view>
							<view v-else>
								<view class="item-select">{{itemValue[detailItem.textName]}}</view>
			...
			inputValidation(){
				...
				this.formTemplate.forEach(el => {
					...
					el.object.forEach(el => {
						...
						if(el.isMustfill && (this.itemValue[el.textName]=="" || this.itemValue[el.textName]==null)){ //必填
